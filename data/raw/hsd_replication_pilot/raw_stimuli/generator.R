...
fillers$filler <- with(fillers, paste(gen, poss, converb, obj , adv, mv, sep = " "))
fillers$item <- 101:160
...
stim_fill$ibex_sentence <- with(stim_fill, sprintf('[["%s", %d], "DashedAcceptabilityJudgment", {s: "%s"}]', condition, item, sentence))
